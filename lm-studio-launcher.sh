...
if [[ "${1:-}" == "--seed" ]]; then
  [[ -n "${2:-}" ]] || { echo "Usage: lm-studio --seed <URL>"; exit 1; }
...
if (( WAIT_UPDATE == 1 )); then
  ...
  if [[ -z "$latest_url" ]]; then
    echo "[lmstudio] ERROR: Could not resolve latest installers URL. Try: lm-studio --seed 'https://lmstudio.ai/download/latest/linux/x64'"; exit 1
  fi
